...
class Convolution1D(nn.Module):
    ...
    def __init__(self, size_in, length, size, stride=1):
        ...
        self.Conv = nn.Conv1d(self.size_in, self.size, self.length, stride=self.stride, padding=self.length)
        # use Glorot uniform initialization
        self.Conv.weight.data = init.glorot_uniform((self.size, self.size_in, self.length))
